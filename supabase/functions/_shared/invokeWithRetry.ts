...
export async function invokeWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (result.data && result.data.success === false && !result.data.canceled) {
        throw new Error(`Function error: ${result.data.error || result.data.message || 'Unknown error'}`);
      }

      console.log(`✅ ${functionName} invoked successfully (attempt ${attempt + 1})`);
...
export async function invokeAgentWithRetry(
  ...
  // Only include analysisContext if it's provided
  if (analysisContext) {
  ...
  try {
    ...
    if (error) {
      console.error(`⚠️ RPC failed to set status for ${agentDisplayName}:`, error);
      console.log(`🔄 Attempting fallback status update for ${agentDisplayName}`);
      
      // Try using the atomicUpdate function directly
      ...
      if (!updateResult.success) {
        console.error(`❌ Fallback also failed for ${agentDisplayName}:`, updateResult.error);
      } else {
        console.log(`✅ Fallback succeeded - ${agentDisplayName} set to running`);
      }
    } else {
  ...
  setupCoordinatorTimeoutMonitoring(supabase, agentName, analysisId, ticker);
...
function setupCoordinatorTimeoutMonitoring(
  ...
  supabase
    ...
    .catch(err => console.error('Failed to get initial updated_at:', err));
        ...
        if (initialUpdatedAt && currentUpdatedAt === initialUpdatedAt) {
          ...
          if (!hasAgentInsight) {
            ...
            await supabase
              ...
              .catch(err => console.error('Failed to log timeout warning:', err));
          }
        } else {
