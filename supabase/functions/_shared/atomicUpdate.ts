      ...
      try {
        ...
        let updateQuery = supabase
          ...
          .update({
            ...
            // Update the proper analysis_status field
...
export async function setAgentToError(
  ...
  try {
    ...
    const insightsResult = await updateAgentInsights(
      supabase,
      analysisId,
      `${agentName.toLowerCase().replace(/\s+/g, '')}_error`,
      errorInsight
    );
    ...
    // DO NOT set analysis_status to ERROR here!
    // Let the analysis-coordinator decide based on agent criticality and phase health
    // The coordinator has the logic to determine if the workflow should stop or continue
    console.log(`⚠️ Agent ${agentName} marked as error - notifying coordinator to decide workflow fate`);

    // CRITICAL: Notify the coordinator about the agent error
    // The coordinator will decide whether to continue or stop
    ...
    if (ticker && userId && apiSettings) {
      ...
      try {
        ...
        const agentFunctionName = agentName.toLowerCase().replace(/\s+/g, '-');
        const fullAgentName = agentFunctionName.startsWith('agent-') ? agentFunctionName : `agent-${agentFunctionName}`;
    ...
    return { 
      success: statusResult.success && insightsResult.success && messageResult.success,
      statusResult,
      insightsResult,
      messageResult,
