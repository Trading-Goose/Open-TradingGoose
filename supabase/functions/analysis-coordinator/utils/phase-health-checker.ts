...
function evaluatePhaseReadiness(
  ...
  switch (phase) {
    case 'analysis':
      // Analysis phase: Can proceed if at least 50% succeeded
      const analysisSuccessRate = status.successfulAgents / status.totalAgents;
      if (analysisSuccessRate < 0.5) {
        status.reason = `Insufficient success rate: ${(analysisSuccessRate * 100).toFixed(0)}%`;
        return false;
      }
      return true;
    ...
    case 'portfolio':
      ...
      if (status.pendingAgents > 0) {
        status.reason = 'Portfolio Manager not started';
        return false;
      }
      if (status.failedAgents > 0) {
        status.reason = 'Portfolio Manager failed';
        return false;
      }
      if (status.runningAgents > 0) {
        status.reason = 'Portfolio Manager still running';
        return false;
      }
      // Only proceed if portfolio manager completed successfully
      if (status.successfulAgents === 0) {
        status.reason = 'Portfolio Manager not completed';
        return false;
      }
      return true;
