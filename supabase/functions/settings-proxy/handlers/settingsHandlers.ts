...
export async function handleUpdateSettings(supabase: SupabaseClient, userId: string, body: any): Promise<Response> {
  ...
  for (const [key, value] of Object.entries(cleanedSettings)) {
    // Check if this is an API key field and it's a new value (not masked)
    if (key.includes('api_key') && value && !isMaskedValue(value)) {
    ...
    if (key === 'alpaca_paper_api_key' && value && !isMaskedValue(value)) {
    ...
    if (key === 'alpaca_live_api_key' && value && !isMaskedValue(value)) {
