        ...
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              {analysisData?.status === ANALYSIS_STATUS.CANCELLED && (
                <Badge variant="pending" className="text-sm">
                  <XCircle className="w-3 h-3 mr-1" />
                ...
                return shouldShow && (
                  <div className="px-6 py-4 bg-muted/50 border-b">
                    <div className="flex items-center justify-between">
                      ...
                      {(displayDecision || analysisData?.status === ANALYSIS_STATUS.CANCELLED) && (
                        <Badge
                          variant={getDecisionVariant(analysisData.status === ANALYSIS_STATUS.CANCELLED ? 'CANCELED' : displayDecision)}
                          className="text-sm px-3 py-1 flex items-center gap-1"
                        >
                          {getDecisionIcon(analysisData.status === ANALYSIS_STATUS.CANCELLED ? 'CANCELED' : displayDecision)}
                          {analysisData.status === ANALYSIS_STATUS.CANCELLED ? 'CANCELED' : displayDecision}
                        </Badge>
                      )}
                    </div>
                  </div>
