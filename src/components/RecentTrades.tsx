  ...
  const fetchAIDecisions = async () => {
    ...
    try {
      ...
      const { data: tradingActions, error: tradingActionsError } = await supabase
        ...
        .select('id, analysis_id, alpaca_order_id, metadata, status, ticker, action, shares, price, dollar_amount, reasoning, created_at, source_type')
        .eq('user_id', user.id)
      ...
      // Only include orders that have an associated analysis
