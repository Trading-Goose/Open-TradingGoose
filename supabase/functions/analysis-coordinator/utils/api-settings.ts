...
export function getAgentSpecificSettings(
  ...
  if (teamConfig) {
    ...
    } else if (teamProviderId === null) {
      // null provider ID means use the default provider
      // Look for the default provider (the one from api_settings)
      const defaultProvider = Object.values(providerMap).find(p => p.is_default === true);
      if (defaultProvider) {
        teamProviderConfig = defaultProvider;
        console.log(`   🔍 Using default provider (provider_id is null)`);
      }
    }

    if (!teamProviderConfig && teamAiProvider) {
      // Fallback to provider name lookup (backward compatibility)
      teamProviderConfig = providerMap[teamAiProvider];
      console.log(`   🔍 Looking for provider by name: ${teamAiProvider}`);
    }
    ...
    if (teamProviderConfig) {
      ...
      console.warn(`   ⚠️ No provider config found, keeping default`);
