  ...
  for (let i = 0; i < stocks.length; i += batchSize) {
    ...
    await Promise.all(
      batch.map(async (stock) => {
        try {
          // Alpaca bars endpoint for stock prices
          ...
          const url = `${baseUrl}/v2/stocks/${stock.ticker}/bars?${params}`;
          ...
          // Alpaca returns data in format: { bars: { SYMBOL: [...] }, next_page_token, symbol }
          // or { bars: [...] } depending on the endpoint version
          let bars = data.bars?.[stock.ticker] || data.bars || data[stock.ticker] || [];
