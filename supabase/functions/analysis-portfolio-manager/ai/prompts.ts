...
export function generateIndividualAnalysisPrompt(
  ...
  const getPositionStatus = (position: any) => {
    ...
    if (plPercent >= gainReview) return '🎯 REVIEW THRESHOLD BREACHED';
    if (plPercent >= gainReview * 0.8) return '📊 APPROACHING REVIEW LEVEL';
    if (plPercent <= -lossGuard) return '🔥 LOSS GUARD BREACHED';
    if (plPercent <= -lossGuard * 0.8) return '⚠️ DRAWDOWN DEVELOPING';
  ...
  const getSizeStatus = (position: any) => {
    ...
    if (positionValue >= numericMaxPositionValue) return 'MAX SIZE';
    if (positionValue >= numericMaxPositionValue * 0.8) return 'NEAR MAX';
    if (positionValue <= numericMinPositionValue) return 'MIN SIZE';
    if (positionValue <= numericMinPositionValue * 1.2) return 'NEAR MIN';
    return '';
  };
