...
serve(async (req) => {
  ...
  try {
    ...
    if (!cancellationCheck.shouldContinue) {
      console.log(`🛑 Social Media Analyst stopped: ${cancellationCheck.reason}`);
      return new Response(JSON.stringify({
        success: false,
        message: `Social Media Analyst stopped: ${cancellationCheck.reason}`,
    ...
    try {
      const data = await callPerplefina({
        focusMode: 'social',
        query: `Social media sentiment analysis for ${ticker} stock including Reddit, Twitter, and trading communities discussion`,
