...
export async function selfInvoke(
  ...
  if (result.error) {
    throw new Error(`Self-invocation failed: ${result.error.message || JSON.stringify(result.error)}`);
  }
...
export async function handleRetryFailure(
  ...
  try {
    ...
    // As a last resort, try to mark the analysis as failed directly
    try {
      await supabase
      ...
      console.log(`📝 ${agentName}: Marked analysis as failed directly in database`);
    } catch (dbError) {
      console.error(`💥 ${agentName}: Critical failure - unable to record failure anywhere:`, dbError);
    }
  }
}
